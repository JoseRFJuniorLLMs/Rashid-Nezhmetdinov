...
let tree_manipulation_props = {
    ...
    console.log("=== MAKE_MOVE DEBUG ===");
    console.log("Move:", s);
    console.log("Original node:", original_node);
    console.log("Original table:", original_node.table);
...
make_move: function(s) {
    ...
    // Aplicar efeito visual ANTES de fazer o movimento
    ...
    console.log("=== DEBUG MAKE_MOVE ===");
    console.log("original_node.table:", original_node.table);
    console.log("this.node.table:", this.node.table);
    
    // Avaliar qualidade do lance
    if (original_node.table && this.node.table) {
        
        try {
            let prevScore = original_node.table.get_eval();
            let currScore = this.node.table.get_eval();
            ...
            if (prevScore !== null && currScore !== null && 
                typeof prevScore === 'number' && typeof currScore === 'number') {
                
                let prevCP = (prevScore - 0.5) * 200;
                let currCP = (currScore - 0.5) * 200;
                
                console.log("prevCP:", prevCP);
                console.log("currCP:", currCP);
                console.log("color:", original_node.board.active);
                
                let quality = evaluateMoveQuality_nibbler(
                    prevCP, 
                    currCP, 
                    original_node.board.active
                );
                ...
                if (quality) {
                    setTimeout(function() {
                        console.log("Chamando showMoveQualityPopup_nibbler");
                        showMoveQualityPopup_nibbler(quality);
                    }, 400);
                }
            }
        } catch (e) {
