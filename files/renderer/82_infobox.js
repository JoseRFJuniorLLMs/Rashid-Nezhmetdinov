...
let infobox_props = {
	...
	draw_infobox: function(node, mouse_point, active_square, active_colour, hoverdraw_div, allow_inactive_focus, lookup_object) {
		...
		let info_line_count = 0;

		for (let info of info_list) {
			if(info_line_count >= 3) {
				break;
			}
			info_line_count++;
			...
			if(info_line_count <= 2) {
				divclass += " infoline_border";
			}
			...
			for (let i = 0; i < nice_pv.length; i++) {
				...
				if(numstring !== "") {
					substrings.push(`<span id="infobox_${clicker_index++}" class="${spanclass}">${numstring}</span>`);
				}

				if(nice_pv[i].includes("N")) { additional_class += "knight" } else
				if(nice_pv[i].includes("Q")) { additional_class += "queen" } else
				if(nice_pv[i].includes("R")) { additional_class += "rook" } else
				if(nice_pv[i].includes("B")) { additional_class += "bishop" } else
				if(nice_pv[i].includes("K")) { additional_class += "king" }

				if(additional_class !== " ") {
					additional_class += color_class + " character_pieces";
				} else {
					additional_class = "";
				}

				substrings.push(`<span id="infobox_${clicker_index}" class="${spanclass}${additional_class}">${nice_pv[i].replace(/N|Q|R|B|K/g, '')} </span>`);
